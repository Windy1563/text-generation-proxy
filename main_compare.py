...
first_endpoint = 'https://jexw6y4ri582oy-5000.proxy.runpod.net'
second_endpoint = 'https://vjwguyhcbueutr-5000.proxy.runpod.net'
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            responses = await asyncio.gather(
                make_request(first_endpoint + "/v1/completions"),
                make_request(second_endpoint + "/v1/completions")
            )

        current_timestamp = datetime.utcnow().timestamp()
        resp_json = {
            "id": f"combined-{current_timestamp}",
            ...
            "usage": {
                "prompt_tokens": responses[0]["usage"]["prompt_tokens"] + responses[1]["usage"]["prompt_tokens"],
                "completion_tokens": responses[0]["usage"]["completion_tokens"] + responses[1]["usage"][
                    "completion_tokens"],
                "total_tokens": responses[0]["usage"]["total_tokens"] + responses[1]["usage"]["total_tokens"]
            }
        }
